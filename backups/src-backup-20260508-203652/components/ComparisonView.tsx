...
function SideBySideView({
  ...
  return (
    <Box sx={{ display: 'flex', gap: 0, width: '100%' }}>
      {/* Left: Screenshot */}
      <Box sx={{ flex: 1, p: 1, borderRight: '1px solid #333', position: 'relative' }}>
        <Typography
          variant="caption"
      ...
      <Box sx={{ flex: 1, p: 1, position: 'relative' }}>
        ...
        <Box
          ref={rightRef}
          sx={{ position: 'relative', cursor: 'crosshair', minHeight: 200 }}
...
/* ============================================================
   Slider View (unchanged, without annotations)
   ============================================================ */
const SliderView = React.forwardRef<
  HTMLDivElement,
  {
    screenshotSrc: string;
...
>(({ screenshotSrc, designImageSrc, sliderPos, isDragging, onMouseDown, onMouseMove }, ref) => {
  const containerRef = useRef<HTMLDivElement>(null);

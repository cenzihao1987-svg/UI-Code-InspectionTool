...
function AppInner() {
  ...
  const handleEditAnnotation = useCallback((id: number) => {
  ...
  // Update annotation handler (for dialog)
  const handleUpdateAnnotation = useCallback(
    (data: { type: Annotation['type']; title: string; description: string; severity: Annotation['severity'] }) => {
      if (editingAnnotation) {
        ...
        setDialogOpen(false);
        setSnackbar({ open: true, message: `标注 #${editingAnnotation.id} 已更新`, severity: 'success' });
      }
    },
    [editingAnnotation]
  );
